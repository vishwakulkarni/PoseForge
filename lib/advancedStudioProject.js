...
const DATA_SANITIZERS = {
  ...
  imageInput(value) {
    ...
    const characterId = shortText(value.characterId, 64);
    if (characterId && UUID_PATTERN.test(characterId)) data.characterId = characterId;
...
function sanitizeNode(value) {
  ...
  const expandedWidth = finiteNumber(value.expandedWidth, 0, 0, 6_000);
  const expandedHeight = finiteNumber(value.expandedHeight, 0, 0, 6_000);
  if (expandedWidth > 0) node.expandedWidth = expandedWidth;
  if (expandedHeight > 0) node.expandedHeight = expandedHeight;
  return node;
}
