const { sanitizeAdvancedSettings } = require("./studioSettings");
...
const NODE_TYPES = new Set(["character", "pose", "generate", "result", "prompt", "assistant", "group", "note"]);
const ASSET_TYPES = new Set(["character", "pose", "generation", "upload"]);
const IMAGE_FITS = new Set(["fit", "fill"]);
...
function sanitizeNode(value) {
  ...
  if (value.custom === true) node.custom = true;
  ...
  const lastExpandedWidth = finiteNumber(value.lastExpandedWidth, 0, 0, 4_000);
  const lastExpandedHeight = finiteNumber(value.lastExpandedHeight, 0, 0, 4_000);
  if (lastExpandedWidth > 0) node.lastExpandedWidth = lastExpandedWidth;
  if (lastExpandedHeight > 0) node.lastExpandedHeight = lastExpandedHeight;

  const imageFit = shortText(value.imageFit, 16);
  if (IMAGE_FITS.has(imageFit)) node.imageFit = imageFit;
  const label = shortText(value.label, 120);
  if (label) node.label = label;
  if (value.labelEdited === true) node.labelEdited = true;
  const meta = shortText(value.meta, 160);
  if (meta) node.meta = meta;
  const imageUrl = shortText(value.imageUrl, 2_048);
  if (imageUrl) node.imageUrl = imageUrl;
  const assetType = shortText(value.assetType, 32);
  if (ASSET_TYPES.has(assetType)) node.assetType = assetType;
  const assetId = shortText(value.assetId, 128);
  if (assetId) node.assetId = assetId;

  const status = shortText(value.status, 16);
  if (NODE_STATUSES.has(status)) node.status = status;
  const generationId = shortText(value.generationId, 64);
  if (generationId && UUID_PATTERN.test(generationId)) node.generationId = generationId;

  if (kind === "generate") {
    const prompt = shortText(value.prompt, 2_000);
    if (prompt) node.prompt = prompt;
    const engine = shortText(value.engine, 32);
    if (engine) node.engine = engine;
    if (value.advancedSettings && typeof value.advancedSettings === "object") {
      node.advancedSettings = sanitizeAdvancedSettings(value.advancedSettings, 4);
    }
  }

  if (kind === "prompt" || kind === "note") {
    const text = shortText(value.text, 2_000);
    if (text) node.text = text;
  }

  if (kind === "note") {
    const color = shortText(value.color, 16);
    if (/^[a-z]+$/.test(color)) node.color = color;
  }

  if (kind === "group") {
    // Group nodes are pure visual frames: id/position/width/height/label
    // above already cover everything they need.
  }

  if (kind === "assistant") {
    const instruction = shortText(value.instruction, 2_000);
    if (instruction) node.instruction = instruction;
    const outputText = shortText(value.outputText, 2_000);
    if (outputText) node.outputText = outputText;
    const engine = shortText(value.engine, 32);
    if (engine) node.engine = engine;
  }
...
function sanitizeStudioDocument(input) {
  ...
  return {
    ...
    nodes,
    edges,
    ...(source.edgeState === "explicit" ? { edgeState: "explicit" } : {}),
