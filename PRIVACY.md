...
- Metadata, settings, generation history, and saved credentials are stored in
  embedded PGlite by default, or in the configured PostgreSQL server.
...
- Embedded database data persists under `storage/pglite` until the user removes
  it. External PostgreSQL data follows that server's retention policy.
...
records from the database. Provider engines and installed CLIs may collect their
...
- Protect `.env`, `storage/`, external database credentials, backups, and exported logs.
