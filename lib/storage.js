...
function getGenerationOutputPath(generationId, mediaKind = "image") {
  return `generations/${generationId}/output.${mediaKind === "video" ? "mp4" : "png"}`;
}
function getGenerationVideoOutputPath(generationId) { return getGenerationOutputPath(generationId, "video"); }
...
// Advanced Studio image inputs live per project so deleting a project can
// reclaim its uploads in one directory removal.
function getAdvancedAssetPath(projectId, assetId, extension) { return `advanced/${projectId}/${assetId}${ext(extension)}`; }
function getAdvancedProjectDir(projectId) { return `advanced/${projectId}`; }
...
function absolutePathFromPublicUrl(url) {
  ...
  return absolutePath(value.slice("/storage/".length));
}
...
module.exports = { STORAGE_ROOT, ensureStorage, getCharacterPhotoPath, getCharacterProfileViewPath, getCharacterProfileSheetPath, getGenerationPosePath, getGenerationOutputPath, getGenerationVideoOutputPath, getGenerationCharacterPath, getGenerationCharacterReferencePath, getPassportRawPath, getPassportSheetPath, getDocumentRawPath, getDocumentOutputPath, getDocumentSheetPath, getUploadStagingPath, getPoseLibraryPath, getAdvancedAssetPath, getAdvancedProjectDir, persistUpload, absolutePath, publicUrl, absolutePathFromPublicUrl, removeRelative };
