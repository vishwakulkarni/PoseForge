...
/** Runs one Advanced Studio video generation to completion. Frames are copied
 * into the generation folder before queueing, matching the image runner's
 * self-contained history records. */
async function createAdvancedVideoGeneration({
  engineKey,
  engine,
  engineModel,
  prompt,
  ...
  const trimmedPrompt = String(prompt || "").trim().slice(0, MAX_PROMPT_LENGTH);
  if (!trimmedPrompt) throw new Error("Add a prompt before generating.");
  if (typeof engine.generateVideo !== "function") {
    throw new Error(`${engine.label} does not support Advanced Studio video generation.`);
  }

  const id = crypto.randomUUID();
  const outputPath = storage.getGenerationVideoOutputPath(id);
  const posePath = storage.getGenerationPosePath(id, ".png");
  await fs.promises.mkdir(path.dirname(storage.absolutePath(outputPath)), { recursive: true });

  let normalizedStartFrame;
  if (startFramePath) {
    await normalizeToPng(startFramePath, storage.absolutePath(posePath));
    normalizedStartFrame = storage.absolutePath(posePath);
  } else {
    await writePromptOnlyPlaceholder(storage.absolutePath(posePath));
  }

  let normalizedEndFrame;
  const normalizedReferences = [];
  const additionalInputs = [
    ...(endFramePath ? [{ kind: "end", sourcePath: endFramePath }] : []),
    ...referencePaths.map((sourcePath) => ({ kind: "reference", sourcePath })),
  ];
  for (const [index, input] of additionalInputs.entries()) {
    const relPath = storage.getGenerationCharacterPath(id, index + 1, ".png");
    const absolutePath = storage.absolutePath(relPath);
    await normalizeToPng(input.sourcePath, absolutePath);
    if (input.kind === "end") normalizedEndFrame = absolutePath;
    else normalizedReferences.push(absolutePath);
  }
  ...
  const usageEstimate = estimateGenerationUsage({
    ...
    model: engineModel,
    prompt: trimmedPrompt,
    imageCount: (startFramePath ? 1 : 0) + additionalInputs.length,
    quality: resolution,
    aspectRatio,
  });
  const settings = {
  ...
  await pool.query(
    `INSERT INTO generations (id, pose_photo_path, engine, prompt, studio_mode, advanced_settings, usage_metrics, media_kind)
     VALUES ($1,$2,$3,$4,$5,$6::jsonb,$7::jsonb,$8)`,
    [id, posePath, engineKey, trimmedPrompt, "advanced", JSON.stringify(settings), JSON.stringify(usageEstimate), "video"]
  );
  ...
  enqueue(id, async () => {
    try {
      await pool.query("UPDATE generations SET status = 'running', started_at = now() WHERE id = $1", [id]);
      logger.info("advanced studio video started", { requestId, generationId: id, model: engineModel });
      const engineResult = await engine.generateVideo({
        model: engineModel,
        prompt: trimmedPrompt,
        startFramePath: normalizedStartFrame,
        endFramePath: normalizedEndFrame,
        referencePaths: normalizedReferences,
        duration,
        aspectRatio,
        resolution,
        sound,
        outputPath: storage.absolutePath(outputPath),
        apiKey: await getSetting("fal_api_key"),
      });
      const usage = mergeActualUsage(usageEstimate, engineResult?.usage);
      await pool.query(
        "UPDATE generations SET status = 'completed', output_path = $2, usage_metrics = $3::jsonb, completed_at = now() WHERE id = $1",
      ...
      logger.error("advanced studio video failed", { requestId, generationId: id, error: error.message });
  ...
  await jobDone;
  return { id, videoUrl: storage.publicUrl(outputPath) };
}

module.exports = { createAdvancedImageGeneration, createAdvancedVideoGeneration };
