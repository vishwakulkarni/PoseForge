...
const detailSql = `SELECT g.*, c.name AS character_name, bp.name AS background_name, bp.type AS background_type, sp.name AS style_name, sp.type AS style_type, pr.title AS pose_title FROM generations g LEFT JOIN characters c ON c.id = g.character_id LEFT JOIN presets bp ON bp.id = g.background_preset_id LEFT JOIN presets sp ON sp.id = g.style_preset_id LEFT JOIN pose_references pr ON pr.id = g.pose_reference_id`;
function shape(row) { return { id: row.id, status: row.status, characterId: row.character_id, characterName: row.character_name || null, engine: row.engine, characterPhotoUrl: storage.publicUrl(storage.getGenerationCharacterPath(row.id, ".png")), posePhotoUrl: storage.publicUrl(row.pose_photo_path), poseReferenceId: row.pose_reference_id || null, poseTitle: row.pose_title || null, outputUrl: row.output_path ? storage.publicUrl(row.output_path) : null, backgroundPreset: row.background_preset_id ? { id: row.background_preset_id, name: row.background_name, type: "background", isCustom: false } : null, stylePreset: row.style_preset_id ? { id: row.style_preset_id, name: row.style_name, type: "style", isCustom: false } : null, prompt: row.prompt, errorMessage: row.error_message, createdAt: row.created_at, startedAt: row.started_at, completedAt: row.completed_at }; }
...
router.post("/", upload.fields([{ name: "posePhoto", maxCount: 1 }, { name: "characterPhoto", maxCount: 1 }]), asyncHandler(async (req, res) => {
  ...
  logger.debug("generation request received", { requestId: req.requestId, hasPosePhoto: Boolean(pose), poseReferenceId: poseReferenceId || null, hasCharacterPhoto: Boolean(characterPhoto), characterId: req.body.characterId || null, engine: req.body.engine || null });
  try {
    ...
    await pool.query("INSERT INTO generations (id, character_id, character_photo_id, pose_photo_path, pose_reference_id, engine, background_preset_id, style_preset_id, prompt) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)", [id, character?.id || null, characterPhotoRow, posePath, resolvedPoseReferenceId, engineKey, backgroundId, styleId, prompt]);
    enqueue(id, async () => {
      try {
