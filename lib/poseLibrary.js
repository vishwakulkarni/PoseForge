 ...
 */
async function addPoseReference(sourceFilePath, { title, category, isCustom = true } = {}) {
  const id = crypto.randomUUID();
  const relativePath = storage.getPoseLibraryPath(id);
  await normalizeToPng(sourceFilePath, storage.absolutePath(relativePath));
...
async function resolvePoseReferenceFile(id) {
  ...
  await normalizeToPng(target, target);
